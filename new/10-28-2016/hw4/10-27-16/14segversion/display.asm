...
StoreSegTableValue:
    ...
    MOV     AX, WORD PTR CS:ASCIISegTable[BX]                    ; this gets the actual sequence stored at the address of the ALth character in the 14 seg table
    MOV     BX, CX                      ; move value of CX into BX because accessing CX as an element is invalid
    ;MOV     WORD PTR segBuffer[BX], AX           ; move value into segment buffer               
    MOV     BYTE PTR segBuffer[BX], AH
    INC     BX
    MOV     BYTE PTR segBuffer[BX], AL

    ADD     CX,WORDSIZE
    CMP     CX, numSegsBytes                 ; if we have reached capacity of the segment buffer, they'd be equal
    ...
    ;MOV     BYTE PTR DS:segBuffer[BX],AL
...
ClearDisplay:                       ;start clearing the display
    ...
    CMP     BX,numSegsBytes              ; compare counter to 8 (number of segments)
...
StartDisplayMux:					 
    ...
    MOV     DX, currentSeg          ; move address into DX
    ADD     DX, 0000H
    OUT     DX, AL
...
;    
;    MOV     AH, 0               ; clear to use AX correctly;
;	MOV		AX, currentSeg		; we want (muxCounter + 1) m;od (numDigits)
;	INC     AX
    
 ;   MOV     CX, 8d
;	DIV		CX                  ; divide by the number of digits (DX = remainder)
	
;    MOV 	currentSeg, DX		; move modded value back into current mux digit (just the lower byte)
	;JMP EndDisplayMux          ; to wrap around

    
;    AND     AX, ODD_OR_EVEN_BIT_MASK               ;//////////////// bit operations to divide by 2
;    CMP     AX, 1               ; see if remainder when dividing by 2 is 0 or 1 
...
;    MOV     AL, currentSeg      ; find porting address = (currentSeg)/2    
;    
;    MOV     CX, WORDSIZE        ; divide by 2 (wordsize = 2 bytes)
;    DIV     CX                  ; divides AX by 2 to get index of LED display///////////////////// can do bitshift
;    
;    MOV     DX, AX              ; move to DX for address of porting to LED display
;    
;    MOV     BH, 0               ; clear BH
 ;   MOV     BL, currentSeg      
 ;;   MOV     AL, segBuffer[BX]   ; value of code stored at index 
...
; 
;    MOV     DX, 0008H           ; address for higher byte
;    MOV     BH, 0               ; clear BH
;    MOV     BL, currentSeg      
;    MOV     AL, segBuffer[BX]   ; value of the code stored at index 
;                                        ; currentSeg of the segBuffer -> AL
;    OUT     DX, AL
;    ;
;	JMP	IncrementMuxCounter
...
segBuffer	DW	 numSegsBytes DUP (?)	; buffer holding currently displayed pattern
