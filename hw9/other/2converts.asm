...
;	Dec2String (public) - converts a 16-bit representation of a number to its 
; 						decimal representation in the form of a string.
; 	Hex2String (public) - converts a 16-bit representation of a number to its 
; 						hexadecimal representation in the form of a string. 
;
; Revision History:
;     10/10/16    Jennifer Du     initial revision
;     10/13/16    Jennifer Du     writing assembly code 
;     10/16/16    Jennifer Du     commenting
; 	  11/30/16 	  Jennifer Du 	  fixing inefficient code 
...
dec2stringinit:
   MOV BX,bFIVE         ; set counter to 5 (will divide input 5 times)
...
signOfNumber:
   ...
   JMP convertPart 		; and continue with converting 
...
convertPart:
   MOV DX,ZERO             ; clear DX again so DIV can work
...
addDigit:
   ...
   CMP BX,ZERO          ; are we at the end? (eval BX == 0)
...
; Return Value:      String stored at SI.
;
...
startOver:
   ...
   SHR AX,FOURBITS      ; shift AX right 12 bits to get true value of the bits
   SHR AX,FOURBITS
   SHR AX,FOURBITS
