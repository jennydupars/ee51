...
;
; These functions have been written to accomodate only queues with a size that 
; is a power of 2. This simplifies tail and head pointer wrapping. 
;
; Revision History:
...
;                   AX - length of queue (ignored, because this initializes a fixed-length queue)
; Return Value:		None. 
;
...
;
; Algorithms: 		None. 
; Data Structures: 	STRUC: has properties size, address, head pointer,
;					and tail pointer
...
SetSize:
    ...
    CMP     BL, BYTE_EL_SIZE      ; see if size = 1 or = 0
    JE      SizeIsByte			
    JNE     SizeIsWord
    
SizeIsWord:
    
    MOV [SI].elsize, WORDSIZE
    JMP SetHeadPTR 

SizeIsByte:
    
    MOV [SI].elsize, BYTESIZE
    JMP SetHeadPTR

SetHeadPTR:
    MOV [SI].head, 0000H   ; set head to 0

SetTailPTR:
    MOV [SI].tail, 0000H   ; set tail to 0
...
QueueFullInit:
	...
	AND AX, ARRAY_SIZE-1		; this trick performs (tail + 1) mod length 
...
;					we will loop until interrupted, and only dequeue 
;					when there is something available to be dequeued. 
;
; Data Structures: 	STRUC: has properties size, address, head pointer,
;					and tail pointer
;					elsize  - the size of elements to be put in (words or bytes)
...
ReturnWord:
    ...
    ADD BX, WORDSIZE    		; increment the head by 2 bytes 
                                    ; we will get (head + 2) mod length 
...
EnqueueInit:
	MOV CX,AX						; store value to enqueue in CX before 
...
QueueNotFull:
    ...
    CMP [SI].elsize, WORDSIZE    	; compare the size to the size of a word
    JE EnqueueWord               	; if size is 2, then we enqueue a word 
    ; JNE EnqueueByte 
	
EnqueueByte: 
	
    MOV BX, [SI].tail          		; load tail address into BX ////////////////////////////////////////
    MOV [SI].array[BX], CL     		; move CL value into the queue at tail
    ADD BX,BYTESIZE					; BX = tail + 1
	AND BX, ARRAY_SIZE-1	 		; the operation (tail+1) AND (size-1)
									; gives us the wrapped around value 
	MOV [SI].tail, BX				; wrapped around value is stored in tail
...
EnqueueWord:
    ...
    ADD BX, WORDSIZE    		; increment the tail by 2 bytes
                                    ; we will get (tail + 2) mod length 
                                        ; by performing (tail + 2) AND (size - 1) 
    AND BX, ARRAY_SIZE-1          		; mod value will be stored in BX 
