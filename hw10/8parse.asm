...
;                   errorStatus (R/W) - keeps track of whether an error occurred 
; 								during parsing. 							
...
InitParsing:				;setup the state machine
	...
	MOV		CL, currentState					;start in the stored current state
												; (where we left off)
    MOV     errorStatus, NO_PARSE_ERROR_VAL   	; reset error flag  
	
ResetIfInInitialState: 
    CMP     CL, ST_INITIAL      		; if currently in ST_INITIAL, reset variables
    JNE     GetToken					; if not in initial state, move on to get token 
    
    CALL    ResetSharedVariables		; reset variables if in initial state
    ; and then continue to get the tokens
...
CheckForError:						; after action, check for error with error flag
	CMP 	errorStatus, PARSE_ERROR_VAL 	
...
; 					flag (errorStatus), and sets the sign variable to positive 1. 
...
;                   errorStatus (W) - error flag tells us whether there was an error
;	 						during parsing or during attempted output 
; 					currentState (W) - keeps track of what state we are at in the 
...
ResetSharedVariables	PROC 	NEAR 
	...
	MOV 	errorStatus, 0 			; start with no error (nonzero when error occurs)
...
ErrorHandler 	PROC 	NEAR 
	...
	MOV 	errorStatus, PARSE_ERROR_VAL	; but set error flag 
	RET 
ErrorHandler	ENDP 
...
DetectSign: 
	CMP 	AL, POSITIVE_ASCII_SIGN		; compare input symbol to '+'
...
StartSendSerialUpdatedSpeed: 
	...
	MOV 	AL, BYTE PTR [SI]				; then move the character at that address into AL 
	...
	JNE 	SendSerialUpdatedSpeed			; if not, keep going 
...
;
;
SendSerialUpdatedTurretElevation 	PROC 	NEAR 							

	MOV 	BX, 0 
SendSerialUpdatedElvAngle: 
	
	LEA 	SI, Updated_Laser_Elv_Header		; load SI with the header label 
	ADD 	SI, BX 							; then add the index to get to current char being added 
	MOV 	AL, BYTE PTR [SI]				; then move the character at that address into AL 
	CALL 	MakeString 						; to call MakeString with 
	INC 	BX 								; then we increment the register holding current position in the label string 
	...
	CMP 	AL, ASCII_NULL 					; see if we've reached the end of label string 
	JNE 	SendSerialUpdatedElvAngle		; if not, keep going 
	; if equal, start to parse number 		; but if we have reached end of label string, then 
		
	CALL 	GetTurretElevation
	LEA 	SI, stringInProgress			; load up string address to concatenate to 
	ADD 	SI, stringInProgressLength 		; get to the location of the end character 
	
	CALL  	Dec2String 
											; will be null-terminated 
OutputElvAngleSerialString: 											
	LEA 	SI, stringInProgress 			; then load SI with string to be output 
	CALL 	SerialPutString 				; then put string through serial channel 
...
RelSpeedOverflowError: 					; reached when speed carries over or overflows
    CALL    ErrorHandler 				; call error handler, do not set motor speed
...
GetDirectionAngleInBounds: 				; get input angle within 0 and 360
	...
	MOV 	CX, TOTAL_DEGREES           ; take remainder as new angle
...
PositiveRelDirectionAngle: 				; may be greater than 360, so mod it 360
	...
	MOV 	CX, TOTAL_DEGREES       	; divide AX by total number of degrees (360)
...
; 						TOKEN_SPACE - tabs or spaces are ignored 
; 										
...
StateTable	LABEL	TRANSITION_ENTRY
	...
	%TRANSITION(ST_INITIAL, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_SPEED, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_SPEED_SIGN, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_SPEED_NUM, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER	
	%TRANSITION(ST_REL_SPEED, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_REL_SPEED_SIGN, DoNOP)			; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_REL_SPEED_NUM, DoNOP)			; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_DIR, DoNOP)						; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_DIR_SIGN, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_DIR_NUM, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ANG, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ANG_NUM, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ANG_REL, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ANG_REL_NUM, DoNOP)			; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ELV, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ELV_SIGN, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ELV_NUM, DoNOP)				; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_ON, DoNOP)					; TOKEN_SPACE
	...
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_O
	%TRANSITION(ST_ERROR, ErrorHandler) 			; TOKEN_OTHER
	%TRANSITION(ST_LAS_OFF, DoNOP)					; TOKEN_SPACE
    ...
    %TRANSITION(ST_ERROR, DoNOP)					; TOKEN_SPACE
