...
; This file holds functions that manage interrupt service routines. The functions
...
; 	  11/30/16 		Jennifer Du 	 commenting, removing magic numbers 

    
; include files 
$INCLUDE(intrpvec.inc)					; include file for setting up interrupt vectors
$INCLUDE(common.inc)					; include file for commonly used constants 
...
ClrVectorLoop:                  ;loop clearing each vector
        ...
        CMP     SI, INTVECSIZE * FIRST_RESERVED_VEC
        JB      DoStore         ;if before start of reserved field - store it
        CMP     SI, INTVECSIZE * LAST_RESERVED_VEC
...
DoStore:                        ;store the vector
        ...
        MOV     ES: WORD PTR [SI + WORDSIZE], SEG(IllegalEventHandler)

DoneStore:                      ;done storing the vector
        ADD     SI, INTVECSIZE  ;update pointer to next vector (2 words, 4 bytes diff.)
