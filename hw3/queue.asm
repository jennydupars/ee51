...
;					we will loop forever until interrupted, and only dequeue 
...
ReturnByte: 
    ...
    AND [SI].head, CX          		; wrapped around value is stored in head 
    JMP EndDequeue 
...
EndDequeue:			; i can move this stuff here!/////////////////////////////////////////////////
	;MOV CX, ARRAY_SIZE        		; we will get (head + 2) mod length 
    ;DEC CX                      	; by performing (head + 2) AND (size - 1) 
    ;AND [SI].head,CX          		; wrapped around value is stored in head 	
...
QueueNotFull:
	;MOV AX, CX						; move value back into AX
...
EnqueueByte: 
	MOV AX, CX						; move value back into AX///////////////////////////////////////////
    MOV BX, [SI].tail          		; load tail address into BX ////////////////////////////////////////
    MOV [SI].array[BX], AL     		; move AL value into the queue at tail
...
EnqueueWord:
    ...
    AND CX, [SI].tail          		; mod value will be stored in CX 
	; ADD BX, BYTESIZE				; BX = tail + 2 
	; AND BX, ARRAY_SIZE-1			; the AND operation gives us wrapped around 
	; MOV [SI].tail, BX				; value, which is stored in tail 
    MOV [SI].tail, CX          		; move the mod length value into queue.tail
    ;JMP EndEnqueue 
