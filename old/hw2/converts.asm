...
dec2stringinit:
   MOV BX,5             ; set counter to 5 (will divide input 5 times)
   MOV DX,0             ; make DX 0 initially
   
signOfNumber:
   MOV DX,AX            ; save AX before you find sign
   AND AX,8000H         ; get first bit
   CMP AX,8000H         ; test if first bit =1 or =0
...
convertPart:
   ...
   CMP BX,0004H         ; if 2nd iteration (BX=4),
   JZ divide10to3          ; divide by 1000
   CMP BX,0003H         ; if 3rd iteration (BX=3), 
   JZ divide10to2          ; divide by 100
   CMP BX,0002H         ; if 4th iteration (BX=2),   
   JZ divide10to1          ; divide by 10
   CMP BX,0001H         ; if 5th iteration (BX=1),
   JZ divide10to0          ; divide by 1

divide10to4:
   MOV CX,TENto4        ; get ready to divide by 10000
   DIV CX               ; divide number by 10000 
...
addDigit:
   ADD AX,ASCII0        ; add to get ascii value of quotient
   MOV BYTE PTR[SI],AL  ; add ascii of the number to string
   INC SI               ; increment pointer by a word's space
...
;                    indicated by the passed address (a). The number (n) is ;                    passed in AX by value. The address (a) is passed in SI by 
;                    value.
;
...
startOver:
   AND AX,0F000H        ; get first 4 digits, which goes to AX
   SHR AX,FOURBITS      ; shift right 12 bits to get true value of the bits
   SHR AX,FOURBITS
   SHR AX,FOURBITS
   CMP AX,000AH         ; compare digits to 10d
...
lessThanTen:
   ...
   CMP BX,4             ; stop when you've done it 4 times
   JZ hex2StringAddNull           ; If BX=4 you end.
   JMP startOver        ; do this process again if BX != 4

greaterThanTen:         ; if we need an ABCDEF
   SUB AX,000AH         ; AX = AX - 10
   ...
   INC BX               ; increment BX
   CMP BX,4             ; have we done this 4 times?
   JZ hex2StringAddNull           ; if BX=4 you end.
   JMP startOver        ; do this process again if BX != 4
