...
; Include files 
$INCLUDE(keypad.inc)				; defines constants used for keypad reading
$INCLUDE(uievents.inc)				; constants used for event handling and UI
...
;	            pressed key. It is called at every timer interrupt, or 1000 times 
; 				per second. Once it has a debounced key, this function will call 
...
NewKey: 
	...
	JE 		UpdateRow					; if the new key's value matches 0fH,
										...
										; scanning next row 
	JNE 	EndCheckKeypadInput			; if new key is a press, end function and 
										; wait til next time
...
; of number of function calls. we count down to count function calls 
    debounceCount		DW 		?
