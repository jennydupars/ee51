...
;     11/30/16      Jennifer Du      added turret elevation functions
; 	  12/10/16 		Jennifer Du 	 touched up comments 
...
; Shared Variables: pulseWidth (r) - array of pulse width counts corresponding to each motor 
;                   pulseWidthCnt (r/w) - counter to implement pulse width modulation movement 
;                   motorOutVal (r) - value to be written to port B to change I/O devices 
;                   laserStatus (r) - status of laser (on or off) 
...
; 					direction and speed of motion. Controlled by a byte on port B.
;
; Error Handling: 	None. 
;
...
UpdateMotorStatus:
    ...
    ;JNZ     GetAbsValOfNegSpeed ; if sign bit is set, is negative, so get magnitude
	JZ		CompareToPulseWidthCount
...
TurnOnMotor:
    ...
    TEST    AL, NEG_SIGN_SET    	; test to see if sign bit is set (negative number)
    JNZ     SpeedIsNegative 		; if sign bit is set (negative)
...
TurnOffMotor: 			; in the case that PWM count is greater than the PWM value for motor
	...
	SHL 	BX, 1						; multiply by 2 to get even index in table 
    MOV 	AL, motorOutVal 			
...
GetMotorSpeed 	PROC	NEAR
    ...
    SHL     AX, 1                   ; stored robotSpeed in Q0.15 form, return it to 
                                    ; full-range form (0~65534) 
	RET
...
Initvariables:						; set all to 0 for 
    ...
    MOV     turretElevation, 0          ; initialize turret elevation to 0 
...
; add 60 (negative offset)
    MOV     AH, 0 
    MOV     turretElevation, AL         ; store elevation 
...
; multiply by 128 (translate to pwm count)
	MOV     CX, 128
    MUL 	CX 		; get 0, 32, 64, 96, or 128
	
; store value into the turretElevationPWM variable 
	MOV 	turretElevationPWM, AL 
