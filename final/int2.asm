...
; 	  11/30/16 		Jennifer Du 	 new include file to avoid constant duplication
    
; include files 
$INCLUDE(int2.inc)				; contains constants for setting up INT2 interrupts 
$INCLUDE(common.inc)			; commonly used constants 
$INCLUDE(intrpvec.inc)			; contains more general constants used in interrupt vector table setup 
...
StartInstallINT2Handler:
        ...
        MOV     ES: WORD PTR (INTVECSIZE * INT2Vec), OFFSET(INT2EventHandler)
								; increment by 2 to get to the next word
								; then store segment of event handler 
        MOV     ES: WORD PTR (INTVECSIZE * INT2Vec + WORDSIZE), SEG(INT2EventHandler)
